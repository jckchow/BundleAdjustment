...
KNN = 150; % using the # of img points nearest the principal point, assumed to be least effected by distortions
...
%             imagePoints(m,:) = [];
            disp('Missing object space coordinates for this image point')
            ...
            worldPointsID = [worldPointsID; tempID];
        end
    end
...
%                 figure;
%                 plot(imagePointsID(r), residuals(:,1),'b.')
%                 hold on;
%                 plot(imagePointsID(r), residuals(:,2),'r.')
%                 hold off;
...
% % % % % %         
        ...
        residuals = f(x, cx, cy, X, Y, Z, lx, ly);
...
function [v] = f(param, cx, cy, X, Y, Z, lx, ly)

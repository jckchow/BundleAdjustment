...
% radius=0.055; %m medium targets
% radius=0.07/2; %m small targets
circle_color='black';
...
% % % % %     hold off;
% % % % %     axis equal
%     [a, b, newradius, Cx_ab]=Circle_Fitting2(circle, mostInliers, radius, pathname, filename,h);
    [a, b, Cx_ab]=Circle_Fitting3(circle, mostInliers, radius, pathname, filename,h);
...
function [a , b, r, Cx_ab]=Circle_Fitting2(circle_FINAL, inliers, radius, pathname, filename,h)
...
% assign an index number to each observation
obs_index=[1:1:length(circle_FINAL(inliers,1))]';
...
%     disp(string);
    ...
    while (1)
        ...
        num_equ=num_obs/2;
        % number of unknowns
        num_unk=3; % centroid of circle a, b, and radius
        % number of contraints
        num_con=0;
        % redundancy (degrees of freedom)
        redundancy=num_equ-num_unk+num_con;
        ...
        a=mean(circle_FINAL(inliers,1));
        b=mean(circle_FINAL(inliers,2));
        X=[a;b; radius];
        ...
        % Build the design matrix A
        A=zeros(num_equ,num_unk);
        A=[-2.*(l(1:num_equ)-X(1)) -2.*(l(num_equ+1:num_obs)-X(2)) -2.*X(3).*ones(num_equ,1)];
        ...
        % Build the misclosure vector of the observations
        f_xl=zeros(num_equ,1);
        f_xl=(l(1:num_equ,1)-X(1)).^2+(l(num_equ+1:num_obs,1)-X(2)).^2-X(3).^2;
        w_b=B*(l_hat-l)-f_xl;
        ...
        while(iterations_adjustment<100)
            ...
            % Build the design matrix A
            A=[-2.*(l_hat(1:num_equ)-X(1)) -2.*(l_hat(num_equ+1:num_obs)-X(2)) -2.*X(3).*ones(num_equ,1)];
            ...
            % Build the misclosure vector
            f_xl=(l_hat(1:num_equ,1)-X(1)).^2+(l_hat(num_equ+1:num_obs,1)-X(2)).^2-X(3).^2;
            w_b=B*(l_hat-l)-f_xl;
...
plot(circle_x,circle_y,'g');
hold on
plot(circle_FINAL(:,1),circle_FINAL(:,2),'k.');
plot(circle_FINAL(inliers,1),circle_FINAL(inliers,2),'r.');
plot(X(1),X(2),'g+');
plot(l(1:num_blunderless_obs),l(num_blunderless_obs+1:2*num_blunderless_obs),'b.');
% plot(l_hat(1:num_blunderless_obs),l_hat(num_blunderless_obs+1:2*num_blunderless_obs),'c.');
hold off
...
ylabel('Y(m)');
title('Centroid of Circle');
legend(strcat('Best Fit Circle, r=', num2str(X(3),4)),'RANSAC Outliers','Blunders','Centroid','Inliers', 'Location', 'bestoutside');
saveas(h,[pathname,foldername,'Figure_Circle_',filename,'.tif'],'tif')
...
% write unknowns to file
fprintf(out, 'UNKNOWNS: Centroid of Circle (a, b) and radius and Standard Deviation\n');
fprintf(out, '=======================================================================\n\n');
Ouput_Unknowns=[X X_std.*1000];
fprintf(out, '  Centroid [m] \t\t\t Standard Deviation [mm] \n\n');
fprintf(out, [repmat(' %9.9f \t\t\t', 1, size(Ouput_Unknowns,2)), '\n'], Ouput_Unknowns');
...
function [a , b, Cx_ab]=Circle_Fitting3(circle_FINAL, inliers, radius, pathname, filename,h)
...
ylabel('Y(m)');
title('Centroid of Circle');
legend(strcat('Best Fit Circle, r=', num2str(radius,4)),'RANSAC Outliers','Blunders','Centroid','Inliers', 'Location', 'bestoutside');
saveas(h,[pathname,foldername,'Figure_Circle_',filename,'.tif'],'tif')
